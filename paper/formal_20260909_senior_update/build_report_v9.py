...
OUT=ROOT/'output/pdf/FoodStateEdit_Formal_GUO_2530030_SeniorFormat_20260914_v9_no_planar.pdf'
...
def paired_results(c,top):
    ...
    for i,title in enumerate(['Input','ChordEdit [5]','Qwen Image [4]','FoodStateEdit (Ours)']):
        txt(c,title,LX+label+i*(pw+gap)+pw/2,H-top-8,7.5,'ArialB',RED if i==3 else BLACK,align='center')
    y=top+15
    for case,label1,label2 in [('soup','Soup','real'),('rice','Rice','real'),('cake','Cake','synthetic')]:
        ...
        chord=ROOT/'artifacts/day26_chordedit_direct_baseline_v1'/case/'seed_1'
        qwen=ROOT/'artifacts/day27_qwen_image_edit_direct_baseline_v1'/case/'seed_1'
        paths=[base/'input.png',chord/'final.png',qwen/'raw_qwen.png',
               outcome('cake') if case=='cake' else base/'relative3d_final_seed2_f20.png']
        for folder,filename in [(chord,'final.png'),(qwen,'raw_qwen.png')]:
            manifest=folder/'condition_manifest.json'
            frozen=json.loads(manifest.read_text(encoding='utf-8'))
            assert hashlib.sha256((folder/filename).read_bytes()).hexdigest()==frozen['files'][filename]['sha256']
            USED_IMAGES.add(manifest)
        for other in [chord/'input.png',qwen/'input.png',NONDATA/case/'reference.png']:
            assert np.array_equal(np.array(Image.open(paths[0]).convert('RGB')),
                                  np.array(Image.open(other).convert('RGB'))), ('input mismatch',case,other)
        for i,p in enumerate(paths):
            with Image.open(p) as im:
                scale=min(pw/im.width,ph/im.height);iw=im.width*scale;ih=im.height*scale
            image_crop(c,p,None,LX+label+i*(pw+gap)+(pw-iw)/2,H-y-ph+(ph-ih)/2,iw,ih)
        y+=ph+8
...
def first_page(c):
    ...
    y=para(c,'動作は接近（0〜5），接触（6〜8），持上げ（9〜15），停留（16〜20）の21フレームで指定する．道具と食材の接触点，相対深度，形状を同じ時刻に描画し，既存VACEへ渡す操作条件を構築する．追加部分はこの制御構築と局所合成であり，生成モデルの新規学習ではない．',LX,y)
    ...
    yy=para(c,'図2は既使用実画像のスープ・炒飯と合成ケーキの選定例である．同じ入力と取食要求でChordEdit [5]，Qwen Image [4]，提案手法を比較する．提案手法はPAI/Wan2.2-VACE-Fun-A14B，688×512画素，21 frames，20 steps，scale 1，追加学習・TTMなしを用いる．',RX,yy)
...
def second_page(c):
    ...
    y=wide_para(c,'図2  同一入力の外部手法との比較．要求はスープをスプーンですくう，炒飯を一口すくう，ケーキをフォークで持ち上げる操作．ChordEdit・Qwenはseed 1，Oursはスープ・炒飯がseed 2，ケーキがseed 1の既存選定例．Qwenはraw，ChordEdit・Oursは局所合成後であり，同一後処理の消融ではない．画像の縦横比を保持して配置した．',y+1)
    ...
    evidence=json.loads((ROOT/'results/day34_presentation_soup_same_input_metrics_v1.json').read_text(encoding='utf-8'))
    rows=[['手法','PSNR','SSIM','外MAE','外保持','Action','Photo','Pres.']]
    for name,record in zip(['ChordEdit [5]','Qwen Image [4]','FoodStateEdit (Ours)'],evidence['records']):
        assert hashlib.sha256((ROOT/record['path']).read_bytes()).hexdigest()==record['sha256']
        met=record['metrics'];review=record['internal_non_blind_review']
        rows.append([name,f"{met['full_psnr_db']:.2f}",f"{met['full_ssim']:.3f}",f"{met['outside_rgb_mae']:.3f}",
                     f"{100*met['outside_exact_pixel_fraction']:.1f}%"]+
                    ['✓' if review[k] else '×' for k in ['action_success','photo_success','preservation_success']])
    y=table(c,rows,LX,y,
        [104,48,44,55,64,63,62,62.069],highlight=(3,),size=7.3)

    left=head(c,'4.1  結果と解釈',LX,y+2)
    left=para(c,'表1の選定例ではAction・Photo・Preservationを同時に満たした．領域外MAEはα=0の画素におけるRGB絶対差の平均（0〜255）である．差0と完全一致率100%は局所合成の効果であり，VACE単体の背景保持性能ではない．',LX,left)
    left=para(c,'図2では取食状態の配置と周辺領域の保持を示す．Qwenは自然な外観を生成する一方，画角や背景も変化する例がある．提案手法にも道具形状や接触の不自然さが残るため，写真品質全般の優位や全例の成功を意味しない．',LX,left)
    left=para(c,'評価対象は開発に用いた選定例であり，3D制御そのものの追加効果，未使用画像への汎化，統計的な優位性は未実証である．',LX,left)
...
def build():
    import shutil
    (HERE/'fonts_v9').mkdir(parents=True,exist_ok=True)
    fonts();FIG.mkdir(parents=True,exist_ok=True);OUT.parent.mkdir(parents=True,exist_ok=True)
    shutil.copyfile(ROOT/'paper/predefense_20260907_senior_match/fonts/LICENSE',HERE/'fonts_v9/LICENSE')
    ...
    assert not re.search(r'Planar|2D平面|表2|36出力|60出力',pdftext,re.I)
    ...
    (HERE/'report_ja_v9.md').write_text(manuscript_header+'\n\n'+'\n\n'.join(BODYTEXT),encoding='utf-8')
    ...
    (HERE/'layout_audit_v9.json').write_text(json.dumps(dict(reference=str(REFPDF),page_count=2,
        ...
        layout_deviation='Page 2 top is a wide three-material Input / ChordEdit / Qwen Image / Ours figure; other text remains two columns.',
        revision_scope='Remove the internal comparison explanation and table; replace the results figure with existing external-model examples; preserve uncertainty and future work.',
