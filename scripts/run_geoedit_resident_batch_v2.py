#!/usr/bin/env python3
"""Run the exclusive-mask FoodStateEdit pilot in resident GeoEdit workers."""
...
METHOD = "foodstateedit_staged_exclusive"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--projection-mask-root", type=Path, required=True)
...
def verify_projection_masks(mask_dir: Path, proxy_dir: Path) -> dict[str, object]:
    manifest_path = mask_dir / "projection_mask_manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if manifest.get("schema_version") != "foodstateedit.exclusive_projection_masks.v1":
        raise ValueError(f"Unexpected projection-mask schema: {manifest_path}")
    if manifest.get("anchor_id") != mask_dir.name:
        raise ValueError(f"Projection-mask identity mismatch: {manifest_path}")
    if manifest.get("status") != "complete":
        raise ValueError(f"Projection-mask package is incomplete: {manifest_path}")
    if not manifest.get("union_coverage_preserved"):
        raise ValueError(f"Projection masks changed union coverage: {manifest_path}")
    if manifest.get("pairwise_overlap_pixel_count") != 0:
        raise ValueError(f"Projection masks are not disjoint: {manifest_path}")
    if tuple(manifest.get("ownership_priority_high_to_low", ())) != OWNERSHIP_PRIORITY:
        raise ValueError(f"Projection-mask priority mismatch: {manifest_path}")
    source_manifest = proxy_dir / "proxy_manifest.json"
    if sha256_file(source_manifest) != manifest["source_proxy_manifest_sha256"]:
        raise ValueError(f"Projection masks reference another proxy: {manifest_path}")
    for layer, item in manifest["projection_mask_files"].items():
        verify_hash(mask_dir / MASKS[layer], item["sha256"], f"exclusive {layer} mask")
    return manifest
...
def command_for_case(
    ...
    return [
        ...
        "--output", str(output),
        "--motion-signal-mask", str(projection_dir / MASKS["rigid"]),
        "--contact-mask", str(projection_dir / MASKS["contact"]),
        "--material-mask", str(projection_dir / MASKS["material"]),
        "--hole-mask", str(projection_dir / MASKS["hole"]),
...
def build_manifest(
    ...
    return {
        ...
        "run_id": f"{anchor_id}__foodstateedit_staged_exclusive__seed_{args.seed}",
        "case_id": anchor_id,
        "method": METHOD,
        "seed": args.seed,
        "model": {
        ...
        "inference": {
            ...
            "projection_mask_manifest": str(
                (projection_dir / "projection_mask_manifest.json").resolve()
            ),
            "projection_mask_manifest_sha256": sha256_file(
                projection_dir / "projection_mask_manifest.json"
            ),
            "projection_mask_schema": projection["schema_version"],
            "projection_mask_ownership_priority_high_to_low": projection[
                "ownership_priority_high_to_low"
            ],
            "projection_masks_pairwise_disjoint": True,
            "projection_union_coverage_preserved": True,
...
def main() -> None:
    ...
    projection_mask_root = args.projection_mask_root.resolve()
    ...
    for anchor_id in args.anchor:
        ...
        projection_dir = projection_mask_root / anchor_id
        projection = verify_projection_masks(projection_dir, proxy_dir)
        projections[anchor_id] = (projection_dir, projection)
    ...
    for anchor_id in args.anchor:
        ...
        projection_dir, projection = projections[anchor_id]
        run_dir = output_root / anchor_id / f"seed_{args.seed}"
        run_dir.mkdir(parents=True, exist_ok=False)
        output_video = run_dir / "result.mp4"
        command = command_for_case(args, proxy_dir, projection_dir, output_video)
