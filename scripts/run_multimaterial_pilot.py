"""One admitted GPU job, one resident VACE pipeline, eight serial frozen arms."""
import argparse
import contextlib
import json
import os
...
def main():
    ...
    if config['schema_version']!='foodstateedit.multimaterial_execution.v1' or not config['execution_allowed']:
    ...
    manifest=dict(schema_version='foodstateedit.multimaterial_run.v1',status='loading_pipeline',
                  ...
                  preflight_sha256=sha256(args.preflight_report),pipeline_load_count=0,expected_conditions=8,
    ...
    try:
        ...
        assert len(manifest['completed_conditions'])==8
