#!/usr/bin/env python3
"""Launch the frozen non-identity action LoRA pilot after a passing preflight."""
...
def main() -> int:
    ...
    preflight_script = Path(__file__).resolve().with_name("preflight_adapter_action_training.py")
    preflight_command = [
        sys.executable,
    ...
    run_manifest = {
        ...
        "next_required_gate": "same-seed LoRA-off/on comparison on held-out pasta_fork_001",
    }
    (output_root / "run_manifest.json").write_text(json.dumps(run_manifest, indent=2) + "\n", encoding="utf-8")
    print(json.dumps(run_manifest, indent=2))
    return 0 if status == "complete_requires_blind_fork_evaluation" else 3
