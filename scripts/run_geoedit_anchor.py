...
def extract_and_project(video_path: Path, proxy_dir: Path, run_dir: Path) -> dict[str, object]:
    ...
    absolute_difference = np.abs(edited.astype(np.int16) - source.astype(np.int16))
    protected = alpha == 0
    outside_max = int(absolute_difference[protected].max())
    outside_mae = float(absolute_difference[protected].mean())
    feather = np.logical_and(alpha > 0, alpha < 255)
    feather_mae = float(absolute_difference[feather].mean()) if feather.any() else None
    if outside_max != 0:
    ...
    return {
        ...
        "outside_edit_alpha_rgb_mae": outside_mae,
        "feather_band_rgb_mae": feather_mae,
    }
...
def main() -> None:
    ...
    manifest: dict[str, object] = {
        ...
        "environment": {
            ...
            "created_at": now,
            "launcher_file": verify_hash(
                Path(__file__).resolve(), sha256_file(Path(__file__).resolve()), "launcher"
            ),
    ...
    try:
        ...
        manifest["inference"]["outside_edit_alpha_rgb_mae"] = projected[
            "outside_edit_alpha_rgb_mae"
        ]
        manifest["inference"]["feather_band_rgb_mae"] = projected["feather_band_rgb_mae"]
