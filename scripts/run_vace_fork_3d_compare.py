#!/usr/bin/env python3
"""Run the frozen same-seed VACE comparison for the fork 3-D projection."""
...
def verify_file(path: Path, expected: dict[str, object], label: str) -> dict[str, object]:
    if not path.is_file():
    ...
    if actual_size != expected["size_bytes"] or actual_hash != expected["sha256"]:
        raise ValueError(
            f"{label} mismatch: size={actual_size}, sha256={actual_hash}; "
...
def extract_selected_and_project(
    ...
    if len(frames) != expected_frames:
        raise RuntimeError(f"Decoded {len(frames)} frames; expected {expected_frames}")

    source = cv2.imread(str(input_root / "first_frame.png"), cv2.IMREAD_COLOR)
    alpha = cv2.imread(str(input_root / "motion_union_alpha.png"), cv2.IMREAD_GRAYSCALE)
    if source is None or alpha is None:
        raise RuntimeError("Failed to read first_frame.png or motion_union_alpha.png")
    ...
    if alpha.shape != (height, width):
        raise ValueError("Projection alpha dimensions differ from the source")

    raw_path = output_root / "raw_selected_frame.png"
    edited_path = output_root / "edited_2d.png"
    ...
    if outside_max != 0:
        raise AssertionError(f"Exact-protection projection failed: {outside_max}")
    feather = np.logical_and(alpha > 0, alpha < 255)
    return {
        "decoded_frames": len(frames),
        "selected_frame_index": selected_frame_index,
        "outside_motion_support_max_pixel_difference": outside_max,
        "outside_motion_support_rgb_mae": float(difference[protected].mean()),
        "feather_band_rgb_mae": float(difference[feather].mean()) if feather.any() else None,
...
def main() -> int:
    ...
    if str(input_root) != config["control"]["remote_root"]:
        raise ValueError("Input root differs from the frozen config")
    if str(output_root) != config["output_root"]:
        raise ValueError("Output root differs from the frozen config")
    if output_root.exists():
        raise FileExistsError(f"Refusing to reuse output root: {output_root}")

    preflight_script = Path(__file__).resolve().with_name("preflight_vace_fork_3d_compare.py")
    preflight_command = [
        sys.executable,
    ...
    verified_inputs = [
        verify_file(input_root / name, expected, f"control file {name}")
        for name, expected in config["control"]["files"].items()
    ]
    control_manifest = json.loads((input_root / "run_manifest.json").read_text(encoding="utf-8"))
    if (
        control_manifest.get("anchor_id") != config["control"]["anchor_id"]
        or control_manifest.get("geometry_source") != config["control"]["geometry_source"]
        or control_manifest.get("selected_depth_crosses_fork") is not True
    ):
        raise ValueError("Control manifest does not satisfy the frozen 3-D fork contract")
    ...
    manifest: dict[str, object] = {
        "schema_version": "foodstateedit.vace_fork_3d_compare_run.v0",
        "method": METHOD,
    ...
    try:
        ...
        with stdout_path.open("w", encoding="utf-8") as stdout, stderr_path.open("w", encoding="utf-8") as stderr, contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
            ...
            manifest["inference"]["pipeline_load_count"] = 1
        ...
        projection = extract_selected_and_project(
            result_video,
            input_root,
            output_root,
            inference_config["selected_frame_index"],
        ...
        output_paths = [
            result_video,
            projection["raw_selected_frame"],
            projection["edited_2d"],
            stdout_path,
            stderr_path,
        ...
        manifest["status"] = "complete_requires_separate_action_and_photo_review"
        return_code = 0
