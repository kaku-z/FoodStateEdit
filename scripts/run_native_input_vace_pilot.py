#!/usr/bin/env python3
"""Run the frozen Day 22 native-input VACE baseline for one soup case."""
...
SCHEMA = "foodstateedit.native_input_vace_pilot.v1"
RUN_SCHEMA = "foodstateedit.native_input_vace_pilot_run.v1"
CASE_ID = "soup"
...
def main() -> int:
    ...
    if config.get("schema_version") != SCHEMA or not config.get("execution_allowed"):
        raise ValueError("Unexpected or disabled native-input config")
    if config.get("case_id") != CASE_ID or config.get("condition") != CONDITION:
        raise ValueError("The frozen case or condition changed")
    if args.output_root.resolve() != Path(config["output_root"]).resolve():
        raise ValueError("Output root differs from the frozen path")
    if args.preflight_report.resolve() != Path(config["preflight_report"]).resolve():
        raise ValueError("Preflight report differs from the frozen path")
    ...
    expected_inference = {
        ...
        "enable_ttm": False,
        "lora_enabled": False,
        "review_frame_indices": [0, 3, 6, 10, 15, 20],
    }
    ...
    manifest = {
        ...
        "case_id": CASE_ID,
        "condition": CONDITION,
        "control_mode": "reference_image_repeated_for_all_frames",
    ...
    try:
        ...
        with (
            ...
            native_frames = geoedit_inference.load_video_or_image(
                reference_path, height, width, inference["num_frames"]
            )
            ...
            video = pipe(
                ...
                vace_video=native_frames,
                vace_video_mask=mask_frames,
                vace_reference_image=reference,
                vace_scale=inference["vace_scale"],
                enable_ttm=inference["enable_ttm"],
                seed=inference["seed"],
                tiled=True,
            )
            geoedit_inference.save_video(
                video, str(condition_root / "raw.mp4"), fps=inference["fps"], quality=5
            )
            del video, native_frames, mask_frames
        ...
        record = {
            ...
            "vace_scale": inference["vace_scale"],
            "lora_enabled": inference["lora_enabled"],
            "ttm_enabled": inference["enable_ttm"],
