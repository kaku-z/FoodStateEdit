#!/usr/bin/env python3
"""Run the frozen 64-step phase-varying VACE-LoRA overfit sanity experiment."""
...
from preflight_adapter_training import sha256_file
...
def main() -> int:
    ...
    preflight_script = runtime_root / config["launcher"]["preflight"]
    preflight_command = [
        sys.executable,
    ...
    selected_gpu = preflight_report.get("selected_gpu")
    if not preflight_report.get("ready") or selected_gpu is None:
    ...
    model_paths = [str(model_root / path) for path in training["model_paths"]]
    ...
    command_record = {
        ...
        "expected_optimizer_steps": training["expected_optimizer_steps"],
        "command": command,
        "environment": {
            key: environment[key]
            for key in (
    ...
    expected_checkpoint_names = {
        f"step-{step}.safetensors" for step in training["expected_checkpoint_steps"]
    }
    observed_checkpoint_names = {Path(record["path"]).name for record in checkpoints}
    status = (
        "complete_requires_seen_phase_checkpoint_sweep"
        if return_code == 0 and expected_checkpoint_names.issubset(observed_checkpoint_names)
        else "technical_failure_preserved"
    )
    run_manifest = {
        "schema_version": "foodstateedit.phase_action_overfit_run.v1",
        "method": METHOD,
        "experiment_variant": VARIANT,
        "scientific_status": config["scientific_status"],
        "claim_limit": config["claim_limit"],
        ...
        "expected_optimizer_steps": training["expected_optimizer_steps"],
        "expected_checkpoint_names": sorted(expected_checkpoint_names),
        ...
        "target_audit_sha256": config["target_audit"]["sha256"],
        ...
        "next_required_gate": (
            "Evaluate LoRA-off and steps 16/32/48/64 on both seen phase-varying samples; "
            "do not run a blind fork test unless a clear seen-sample semantic gain appears."
        ),
    }
    ...
    return 0 if status == "complete_requires_seen_phase_checkpoint_sweep" else 3
