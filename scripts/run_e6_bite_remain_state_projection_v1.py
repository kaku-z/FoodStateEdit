"""Run the E6 cavity-state projection ablation against frozen E5b controls."""
import argparse
import contextlib
import json
import os
...
def make_sheet(frames):
    ...
    for slot, frame_id in enumerate([0, 8, 9, 12, 15, 20]):
        ...
        draw.text((x + 4, y + 5), f"E6 state projection / f{frame_id}", fill="black")
    return sheet
...
def main():
    ...
    state = {
        "schema_version": "foodstateedit.bite_remain_state_projection_run.v1",
        "status": "initializing",
