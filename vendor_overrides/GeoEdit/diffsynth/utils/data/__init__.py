...
def save_video(frames, save_path, fps, quality=9, ffmpeg_params=None):
    ...
    first_frame = np.asarray(frames[0].convert("RGB"), dtype=np.uint8)
    height, width = first_frame.shape[:2]
    quality = max(0, min(10, quality))
    crf = int(round(35 - quality * 2.5))
    command = [
        ffmpeg,
        "-hide_banner",
        "-loglevel", "error",
    ...
    try:
        for frame in tqdm(frames, desc="Saving video"):
            ...
            if array.shape != first_frame.shape:
                raise ValueError(
                    f"Video frame shape mismatch: {array.shape} != {first_frame.shape}"
                )
